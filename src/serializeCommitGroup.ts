import type { CommitGroup } from "./groupCommits.js";
...
export const serializeCommit = (
	...
	`- ${commit.message} ([${commit.shortHash}](${remote.getCommitUrl(
		commit.hash,
	)}))\n`;
...
export const serializeCommitGroup = function* (
	...
	if (group.tag) {
		yield `## ${group.tag} (${
			group.commit.committer.date.toISOString().split("T")[0]
		})\n\n`;
		for (const [type, commitList] of [...group.commits].sort(
			([a], [b]): number =>
				getPriority(a, types) < getPriority(b, types) ? 1 : -1,
		)) {
